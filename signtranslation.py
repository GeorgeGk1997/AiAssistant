...
class SignTranslation:
	...
	def __init__(self):
		...
		self.keras_predict(model, np.zeros((50, 50), dtype = np.uint8))
	...
	def get_pred_text_from_db(self, pred_class):
		...
		cmd = "SELECT gesture_name FROM gesture WHERE gesture_id="+str(pred_class)
	...
	def get_pred_from_contour(self, contour, thresh):
		...
		pred_probab, pred_class = self.keras_predict(model, save_img)
	...
	def get_img_contour_thresh(self, img):
		...
		if self.isBgCaptured == True:
			frame = self.remove_background(img1)
	...
	def text_mode(self, cam):
		...
		while True:
			...
			elif keypress == ord('r'):  # press 'r' to reset the background
				time.sleep(1)
				self.bgModel = None
